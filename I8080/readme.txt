...
bdos.asm     - An emulator for CP/M programs running under DB. It translates CP/M
               mode calls to the special supervisor calls implemented in DB.
hello.asm    - A "hello, world" program for CP/M, used to test bdos.asm.
loader.asm   - A loader/mover for bdos.asm. Used to form a binary, located at 
               $0000, that boosts the bdos emulator to high memory in a DB run.
portcall.asm - Implements macros for DB supervisor calls. Used by bdos.asm.
runcpm.txt   - Explains the "runcpm" system and program, which is the 
               bdos/loader set that emulates CP/M programs under DB.
               Note that this project was never completed.
