...
const
   ...
   maxlab = 10000;  { number of characters in label }
...
begin
   ...
   for sc := 1 to dir.eatc do begin { read symbols }
      ...
      if sp^.nrva <> 0 then begin { name exists }
         ...
         until b = 0; { until end of string } 
         copysp(sp^.nam, symbuf); { place symbol }
   ...
   while sp <> nil do begin { print symbols }
      ...
      write(sp^.nam^); { write name }
      for i := max(sp^.nam^) to maxlen do write(' '); { pad it }
...
begin
   ...
   { check for COFF symbols }
   if cofnum > 0 then begin { COFF symbols exist }

      writeln;
      writeln('Symbols table format is ''COFF''');
      dumpcoffs

   end;
   { check for GCC stabs symbol table information }
   stbobj := fndobj('.stab   ');
   stsobj := fndobj('.stabstr');
   if stbobj <> nil then begin

      if stsobj = nil then error(einvsyt); { invalid symbols table }
      writeln;
      writeln('Symbols table format is GCC ''STABS''');
      dumpstabs(stbobj, stsobj) { dump the table }
